 ...
 * @todo none
 */
$feedback = ""; #allow feedback to help see reCAPTCHA not correct
...
if(isset($recaptchaTheme))
{//doesn't exist on no-captcha
	echo '
	<script type="text/javascript">
		var RecaptchaOptions = { theme : "' . $recaptchaTheme . '"};
	</scr';
	echo 'ipt>';//adds reCAPTCHA theme options to top of page
}
...
function handle_POST($skipFields,$sendEmail,$toName,$fromAddress,$toAddress,$website,$fromDomain)
{
	...
	{#create email
		...
		/*
		removed email injection code temporarily to troubleshoot, 10/1/2013
		foreach( $_POST as $value ){#Content-Type: is too similar to email injection to allow
		  ...
		  if( stripos($spaceless,'Content-Type:') !== FALSE ){
			echo "<h1>Incorrect form data. Email NOT sent.</h2>";
			echo '<div align="center"><a href="' . THIS_PAGE . '">BACK</a></div>';
			die;
		  }
		}
...
{#sends PHP POST data to a JS array, where it will be picked up and matched to form elements, then elements will be reloaded
	$aSkip = explode(",",$skipFields); #split form elements to skip into array
	echo '<script type="text/javascript" src="includes/contact-lib/util.js"></script>';
...
function dateFeedback($feedbackWanted)
{
	if($feedbackWanted !== false)
	{
		$feedback = "<div class=\"dateFeedback\">Incorrect reCAPTCHA - ";
		$feedback .= "<script type=\"text/javascript\">document.write(new Date().format('h:MM:ss TT'));</scr";
		$feedback .="ipt></div>";
		return $feedback;
	}else{
		return ''; //no feedback, empty string
	}
